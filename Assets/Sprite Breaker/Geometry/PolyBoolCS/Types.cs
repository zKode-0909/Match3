...
namespace SpriteBreakerUtil {
    ...
    public class Segment {
        ...
        public Segment Clone() {
            Segment copy = new Segment() { start = start, end = end, id = id };
            copy.myFill = myFill?.Clone();
            copy.otherFill = otherFill?.Clone();
            return copy;
        }
    ...
    public class SegmentFill {
        ...
        public SegmentFill Clone() { return new SegmentFill() { above = above, below = below }; }
    ...
    /* [System.Diagnostics.DebuggerTypeProxy( typeof( List<Vector2>.List<Vector2>DebugProxy ) )]
    public class List<Vector2> : List<Vector2> {
        public List<Vector2>()
            : base() {
        }

        public List<Vector2>( int capacity )
            : base( capacity ) {
        }
        ...
        public sealed class List<Vector2>DebugProxy {
            private readonly ICollection<Vector2> list;

            [System.Diagnostics.DebuggerBrowsable( System.Diagnostics.DebuggerBrowsableState.RootHidden )]
            public Vector2[] Points {
                get {
                    var items = new Vector2[ list.Count ];
                    list.CopyTo( items, 0 );
                    return items;
                }
            }

            public List<Vector2>DebugProxy( List<Vector2> target ) {
                this.list = target;
            }
        }
    ...
    public class SegmentList : List<Segment> {
        ...
        public SegmentList Clone() {
            SegmentList copy = new SegmentList( Capacity );
            for ( int i = 0, n = Count; i < n; i++ ) copy.Add( this[ i ].Clone() );
