...
namespace SpriteBreakerUtil {
    ...
    public class Intersecter {
        ...
        public Segment segmentCopy( Vector2 start, Vector2 end, Segment seg ) {
            ...
            if ( seg.myFill != null ) {
                copy.myFill.above = seg.myFill.above;
                copy.myFill.below = seg.myFill.below;
            }
            return copy;
        }
        ...
        public void addRegion( List<Vector2> region, float scale=1.0f) {
            if ( !selfIntersection ) {
            ...
            var pt1 = new Vector2();
            var pt2 = region[ region.Count - 1 ] * scale;
        ...
        private EventNode checkIntersection( EventNode ev1, EventNode ev2 ) {
            ...
            if ( !Epsilon.linesIntersect( a1, a2, b1, b2, out intersect ) ) {
                ...
                if ( Mathf.Abs( intersect.alongA ) < Epsilon.eps ) {
                    if ( Mathf.Abs( intersect.alongB + 1 ) < Epsilon.eps ) // yes, at exactly b1
                        eventDivide( ev1, b1 );
                    else if ( Mathf.Abs( intersect.alongB ) < Epsilon.eps ) // yes, somewhere between B's endpoints
                        eventDivide( ev1, intersect.pt );
                    else if ( Mathf.Abs( intersect.alongB - 1 ) < Epsilon.eps ) // yes, at exactly b2
                        eventDivide( ev1, b2 );
                }

                // is B divided between its endpoints? (exclusive)
                if ( Mathf.Abs( intersect.alongB ) < Epsilon.eps ) {
                    if ( Mathf.Abs( intersect.alongA + 1 ) < Epsilon.eps ) // yes, at exactly a1
                        eventDivide( ev2, a1 );
                    else if ( Mathf.Abs( intersect.alongA ) < Epsilon.eps ) // yes, somewhere between A's endpoints (exclusive)
                        eventDivide( ev2, intersect.pt );
                    else if ( Mathf.Abs( intersect.alongA - 1 ) < Epsilon.eps ) // yes, at exactly a2
                        eventDivide( ev2, a2 );
                }
            }
            ...
            while ( !event_root.isEmpty ) {
                ...
                if ( ev.isStart ) {
                        ...
                        if ( ev.seg.otherFill == null ) {
                            ...
                            if ( below == null ) {
                                ...
                                if ( ev.primary == below.primary )
                                    inside = below.seg.otherFill != null ? below.seg.otherFill.above : true;
                                else
                                    inside = below.seg.myFill != null ? below.seg.myFill.above : true;
                            }
                    ...
                    if ( st == null ) {
                        ...
                        // throw new Exception( "Zero-length segment detected; your epsilon is probably too small or too large" );//
                        //    ( ev.seg != null ? ( ev.seg.start.ToString( "F10" ) + ", end: " + ev.seg.end.ToString( "F10" ) ) : Epsilon.eps.ToString( "F10" ) ) );
