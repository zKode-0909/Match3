...
namespace SpriteBreakerUtil {
    ...
    public class SegmentChainer {
        ...
        public List<List<Vector2>> chain( SegmentList segments, float scale=1.0f ) {            
            ...
            foreach ( var seg in segments ) {
                ...
                if ( Epsilon.pointsSame( pt1, pt2 ) ) {
                    //Debug.Log( "Warning: Zero-length segment detected; your epsilon is probably too small or too large" );
                    continue;
                }
            ...
            // if closing chains failed
            if ( regions.Count == 0 && chains.Count > 0 ) {
                // for ( int i = 0; i < chains.Count; i++ ) {}
            }


            // scale points
            if ( Mathf.Abs( scale - 1.0f ) > Mathf.Epsilon ) {
                for ( int r = 0, nr = regions.Count; r < nr; r++ ) {
                    List<Vector2> reg = regions[ r ];
                    for ( int i = 0, np = reg.Count; i < np; i++ ) reg[ i ] *= scale;                    
                }
            }
            return regions;
        }
